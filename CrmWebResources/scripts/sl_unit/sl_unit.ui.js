...
typeof Softline.Unit === "undefined" && (Softline.Unit = {});

Softline.Unit.Buttons = {
    Translete: {
        command: (fctx) => {
    ...
    UpdateFormatImages: {
        command: async (fctx) => {
            ...
            Xrm.Utility.confirmDialog("Update format images ?", async () => await Softline.Unit.Buttons.UpdateFormatImages.updateFormat(fctx, reference));
        },

        updateFormat: async (fctx, reference) => {
            try {
                Xrm.Utility.showProgressIndicator("Loading");
                const updateFormatButton = Softline.Unit.Buttons.UpdateFormatImages;
    ...
    SaveCoordinates: {
        command: fctx => {
            Xrm.Utility.confirmDialog("Save new coordinates ?", () => Softline.Unit.Buttons.SaveCoordinates.saveCoordinates(fctx))
        },
        enable: fctx => {
            return true;
        },
